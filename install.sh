...
step_server_unit() {
  ...
  if [[ "$DRY_RUN" -eq 0 ]] && systemctl list-unit-files | grep -q "picogallery.service"; then
    step "Removing legacy picogallery.service"
    run systemctl stop picogallery.service || true
    run systemctl disable picogallery.service || true
    run rm -f /etc/systemd/system/picogallery.service || true
