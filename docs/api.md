...
| GET    | `/slideshow/events`   | **SSE** stream. Emits `state` (and `display`) events. |
...
Returns the new slideshow state and broadcasts it to all SSE clients. `goto`
with an unknown `id` → `404`. `favorite`/`goto` require `id`.

## Photos

| Method | Path                     | Notes                                            |
|--------|--------------------------|--------------------------------------------------|
| GET    | `/photos?offset&limit`   | `{ photos, total, offset, limit }` page of the playlist. |
...
- `413 IMAGE_TOO_LARGE` when an original exceeds the MB / megapixel guard.
- `502 SOURCE_ERROR` when the source fetch or decode fails.
- `404 NOT_FOUND` / `503 SOURCE_UNAVAILABLE` as applicable.
...
| Method | Path              | Notes                          |
|--------|-------------------|--------------------------------|
| GET    | `/config/display` | Effective display settings.    |
