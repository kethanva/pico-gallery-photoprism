...
export function errorHandler(error: FastifyError, _req: FastifyRequest, reply: FastifyReply): void {
  if (error instanceof ZodError) {
    reply.status(400).send({
      error: { code: 'VALIDATION_ERROR', message: error.issues.map((i) => `${i.path.join('.')}: ${i.message}`).join('; ') },
    });
    return;
  }

  if ((error as Error).message === 'ENGINE_NOT_READY') {
    reply.status(503).send({ error: { code: 'STARTING', message: 'Server is loading sources, please retry shortly' } });
    return;
  }
