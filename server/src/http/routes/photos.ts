...
export function photosRoute(cfg: RootConfig): FastifyPluginAsync {
  return async (app) => {
    ...
    app.get('/photos', async (req) => {
      ...
      const photos = playlist.slice(q.offset, q.limit);
      return { photos, total: playlist.length, offset: q.offset, limit: q.limit };
    });
    ...
    app.get<{ Params: { id: string } }>('/photos/:id/image', async (req, reply) => {
      ...
      if (!source) return reply.status(503).send({ error: { code: 'SOURCE_UNAVAILABLE', message: 'Source not available' } });
      ...
      try {
        ...
        if (err instanceof ImageGuardError) {
          return reply.status(413).send({ error: { code: 'IMAGE_TOO_LARGE', message: (err as Error).message } });
        }
